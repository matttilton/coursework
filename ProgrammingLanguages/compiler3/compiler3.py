...
def error(num):
    ...
    if num == 1: 
        ...
        print >>outfile, 'Expected function'
    elif (num == 38):
        print >>outfile, 'Expected factor'
    elif (num == 39):
        print >>outfile, 'Cant assign to function here'
    exit(0)
...
def enter(tx, k, level, dx):
    ...
    if k == "const":
        ...
        dx += 1
    elif k == "procedure" or k == 'function' or k == 'FUNCTION':
        x = tableValue(id, k, level, dx, "NULL")
    table.append(x)
    return dx
#--------------CONST DECLARATION---------------------------
...
def statement(tx, level, tx0):
    ...
    if sym == "ident":
        ...
        if (kind == 'variable'):
            ...
            error(39) # I need to find/make an error for this
...
def factor(tx, level):
    ...
    if sym == "ident":
        ...
        getsym();
        if (sym != 'ident'):
            error(36) # find/make error for this
        i = position(tx, id)
        if (table[i].kind != 'function'):
            error(37) # find/make  error for this
        getsym()
        gen('INT', 0, 1)
        gen('CAL', level - table[i].level, table[i].adr)
    elif(sym == 'NOT'):
